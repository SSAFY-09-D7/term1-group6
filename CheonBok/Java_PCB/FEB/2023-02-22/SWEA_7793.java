...
public class SWEA_7793 {
	...
	// 악마, 사람의 좌표 값(x,y)과 이동 차수(level)를 저장하기 위한 class
	static class point {
		int x, y, level;
	...
	private static void gameStart(int x, int y) {
		...
		// 기준은 사람 큐의 유무로 한다 --> 사람이 더 이상 움직일 수 없다면 이기든 지든 게임은 종료된 것이기 때문.
		while (!human.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = 0; j < 4; j++) {
					...
					if(0<=nx && nx <N && 0<=ny && ny<M && 
						...
						arr[nx][ny] = 'S'; // S로 만들어 다음에 중복 탐색을 방지 (방문 처리 역할)
